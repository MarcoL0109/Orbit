...
// module does.
export function isReachable(url: string, timeoutMs = 2000): Promise<boolean> {
...
export async function waitUntilReachable(
    url: string,
    options: {totalTimeoutMs?: number; intervalMs?: number; perAttemptTimeoutMs?: number} = {},
): Promise<boolean> {
    const totalTimeoutMs = options.totalTimeoutMs ?? 10_000;
    const intervalMs = options.intervalMs ?? 1_000;
    const perAttemptTimeoutMs = options.perAttemptTimeoutMs ?? 2_000;
