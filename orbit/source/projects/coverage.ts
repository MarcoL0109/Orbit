...
function routeSegments(route: string): string[] {
    return route.split('/').filter(Boolean).filter((segment) => !segment.startsWith(':'));
}
...
function getSubjectClassification(projectRoot: string, file: string, checksums: ChecksumsFile | null) {
    const checksum = checksums?.files[file]?.checksum;
    return checksum ? getFreshClassification(projectRoot, file, checksum) : null;
}

function collectTestSignals(projectMap: ProjectMap, projectRoot: string, checksums: ChecksumsFile | null): TestSignal[] {
    return projectMap.tests.map((test) => {
        const classification = getSubjectClassification(projectRoot, test.file, checksums);
        return {
...
function hasExactTagMatch(subjectTags: string[], tests: TestSignal[]): boolean {
    ...
    return tests.some((test) => test.llmFeatures?.some((tag) => subjectExact.has(normalizeTag(tag))) ?? false);
}
...
function hasBroadTagMatch(subjectBroadTags: Set<string>, tests: TestSignal[]): boolean {
    return tests.some((test) => {
        if (test.llmFeatures) {
            return test.llmFeatures.some((tag) => subjectBroadTags.has(broadTagOf(tag)));
        }
        return test.featureGuess !== null && subjectBroadTags.has(normalizeTag(test.featureGuess));
    });
}

// Heuristic fallback: does any test's *filename* contain every identifying
// segment (route path segments, or the component name)?
function hasFilenameSegmentMatch(normalizedSegments: string[], tests: TestSignal[]): boolean {
    if (normalizedSegments.length === 0) return false;
    return tests.some((test) => normalizedSegments.every((segment) => test.normalizedPath.includes(segment)));
}
...
function hasClassifiedTagSegmentMatch(normalizedSegments: string[], tests: TestSignal[]): boolean {
    if (normalizedSegments.length === 0) return false;
    return tests.some((test) => {
...
function assess(
    ...
    const normalizedSegments = segments.map(normalizeForSubstring).filter(Boolean);
...
export function computeCoverage(projectMap: ProjectMap, projectRoot: string): CoverageReport {
    ...
    for (const route of projectMap.routes) {
        const classification = getSubjectClassification(projectRoot, route.file, checksums);
        if (classification) llmVerifiedSubjectCount++;

        const {status, assessedBy} = assess(classification?.features ?? null, route.featureGuess, routeSegments(route.route), testSignals);

        routes.push({kind: 'route', name: route.route, file: route.file, featureGuess: route.featureGuess, status, assessedBy});
    }

    const assessableComponents = projectMap.components.filter(
        (component): component is ProjectComponent & {featureGuess: string} => component.featureGuess !== null,
    );

    const components: CoverageEntry[] = [];

    for (const component of assessableComponents) {
        const classification = getSubjectClassification(projectRoot, component.file, checksums);
        if (classification) llmVerifiedSubjectCount++;

        const {status, assessedBy} = assess(classification?.features ?? null, component.featureGuess, [component.name], testSignals);

        components.push({kind: 'component', name: component.name, file: component.file, featureGuess: component.featureGuess, status, assessedBy});
    }
    ...
    return {
        ...
        components,
        unassessableComponentCount: projectMap.components.length - assessableComponents.length,
...
export function colorForCoverageStatus(status: CoverageStatus): 'green' | 'yellow' | 'red' {
...
export function describeCoverageEntry(entry: CoverageEntry): string {
    const confidence = entry.assessedBy === 'llm-verified' ? '[verified]' : '[heuristic]';
    const header = `${STATUS_ICON[entry.status]} ${confidence} ${entry.name} (${entry.file})`;

    if (entry.status === 'covered') {
        return header;
    }

    const note = entry.status === 'loosely-covered'
...
export function formatCoverageSummary(report: CoverageReport): string {
    const coveredRoutes = report.routes.filter((entry) => entry.status === 'covered').length;
    const coveredComponents = report.components.filter((entry) => entry.status === 'covered').length;
