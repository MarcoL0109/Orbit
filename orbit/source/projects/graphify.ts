...
function pickInstallCommand(): {tool: string; args: string[]} | null {
    return INSTALL_CANDIDATES.find((candidate) => commandExists(candidate.tool)) ?? null;
}
...
export function installGraphify(): InstallGraphifyResult {
    ...
    if (!picked) {
        return {
            installed: false,
            reason: 'No package manager found (checked uv, pipx, pip3, pip) — install graphify manually: https://github.com/Graphify-Labs/graphify',
        };
    }
    ...
    if (result.error || result.status !== 0) {
        ...
        return {installed: false, reason: `${picked.tool} ${picked.args.join(' ')} failed: ${detail}`};
    }

    return isGraphifyAvailable()
        ? {installed: true}
        : {installed: false, reason: `${picked.tool} reported success but 'graphify' still isn't on PATH — it may need a shell restart.`};
}
...
export function runGraphifyScan(projectRoot: string): GraphifyScanResult {
    ...
    const args = mode === 'extract' ? ['extract', '.', '--code-only'] : ['update', '.'];
    const result = spawnSync('graphify', args, {cwd: projectRoot, encoding: 'utf8'});
    ...
    const summaryLine =
        ...
        ?? lines.map((line) => line.trim()).reverse().find((line) => line && !line.startsWith('Tip:'))
        ?? 'graph updated';
...
function ensureGraphifyGitignored(projectRoot: string): void {
    ...
    try {
        ...
        if (!alreadyIgnored) {
            ...
            fs.writeFileSync(gitignorePath, `${content}${separator}graphify-out/\n`, 'utf8');
        }
    } catch {
