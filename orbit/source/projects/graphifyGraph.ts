...
export function loadGraphifyGraph(projectRoot: string): GraphifyGraph | null {
    ...
    try {
        const graph = JSON.parse(fs.readFileSync(graphPath, 'utf8')) as GraphifyGraph;
...
export function findGraphNodes(graph: GraphifyGraph, query: string): GraphifyNode[] {
...
export function buildConnections(graph: GraphifyGraph, node: GraphifyNode): GraphConnection[] {
    ...
    for (const link of graph.links) {
        if (link.source === node.id) {
            ...
            connections.push({
                ...
                otherSource: target ? `${target.source_file}:${target.source_location}` : 'unknown',
            });
            ...
            connections.push({
                ...
                otherSource: source ? `${source.source_file}:${source.source_location}` : 'unknown',
            });
        }
    }
