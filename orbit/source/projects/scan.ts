...
function extractSignalsFromContent(content: string) {
	...
	const exports = [
		...preview.matchAll(
			/export\s+(?:default\s+)?(?:async\s+)?(?:function|const|type|class|interface)\s+(\w+)/g,
		),
	].map(match => match[1]!);

	const commandNames = [...preview.matchAll(/name:\s*['"]([^'"]+)['"]/g)].map(
		match => `command:${match[1]}`,
	);

	const usages = [...preview.matchAll(/usage:\s*['"]([^'"]+)['"]/g)].map(
		match => `usage:${match[1]}`,
	);

	const jsxText = [...preview.matchAll(/>([^<>{}]{3,80})</g)]
...
function getNextAppRoute(file: string) {
	const homeMatch = /^(src\/)?app\/page\.(tsx|ts|jsx|js)$/.exec(file);

	if (homeMatch) {
		return '/';
	}

	const match = /^(src\/)?app\/(.+)\/page\.(tsx|ts|jsx|js)$/.exec(file);
...
function getNextPagesRoute(file: string) {
	const match = /^(src\/)?pages\/(.+)\.(tsx|ts|jsx|js)$/.exec(file);
...
function getSvelteKitRoute(file: string) {
	const match = /^(src\/)?routes\/(.*\/)?\+page\.(svelte|ts|js)$/.exec(file);
	...
	const routePart = dirPart
		...
		.map(segment => segment.replace(/\[(\.{3})?(.+?)]/g, ':$2'))
		.join('/');
...
function getRemixRoute(file: string) {
	const match = /^app\/routes\/(.+)\.(tsx|ts|jsx|js)$/.exec(file);
