...
import type { EnvironmentSetupContext, ToolDefinition, ToolResult } from './types.js';
...
// error).
export const runCommandTool: ToolDefinition<RunCommandArgs, RunCommandResult, EnvironmentSetupContext> = {
    name: 'run_command',
    description:
    ...
    parameters: {
        ...
        properties: {
            command: {
                type: 'string',
                description: 'The exact shell command to run, e.g. "docker compose up -d" or "uv run alembic upgrade head".',
            },
            reason: {
                type: 'string',
                description: 'One sentence on why this command is needed right now, shown to the user alongside the command.',
            },
        },
        required: ['command', 'reason'],
    },
    execute: async ({command, reason}, context) => {
        const approved = await context.requestApproval(`Run: ${command}\n(${reason})`);
        if (!approved) {
        ...
        return new Promise<ToolResult<RunCommandResult>>((resolve) => {
            ...
            after(COMMAND_TIMEOUT_MS, () => {
                ...
                after(5000, () => {
                    ...
                    after(5000, () => {
                        if (settled) return;
                        settled = true;
                        resolve({ok: true, data: {stdout: truncate(stdout), stderr: truncate(stderr), exitCode: null}});
                    });
                });
            });
            ...
            child.on('exit', (exitCode) => {
                ...
                after(50, () => {
                    if (settled) return;
                    settled = true;
                    resolve({ok: true, data: {stdout: truncate(stdout), stderr: truncate(stderr), exitCode}});
                });
            });
