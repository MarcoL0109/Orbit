...
// afterward. There's no harm in this being called and being wrong, so it
// only ever needs a best-effort summary of what was attempted.
export const signalEnvironmentReadyTool: ToolDefinition<SignalEnvironmentReadyArgs, SignalEnvironmentReadyResult, EnvironmentSetupContext> = {
    ...
    parameters: {
        ...
        properties: {
            notes: {
                type: 'string',
                description: 'A short summary of what you did to bring the environment up, for the record if reachability still fails afterward.',
            },
            setupProcedure: {
                type: ['string', 'null'],
                description:
        ...
        required: ['notes', 'setupProcedure'],
    },
    execute: async ({notes, setupProcedure}) => ({ok: true, data: {notes, setupProcedure: setupProcedure ?? undefined}}),
};
