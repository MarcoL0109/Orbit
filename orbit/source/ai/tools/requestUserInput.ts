	...
	parameters: {
		...
		properties: {
			prompt: {
				type: 'string',
				description:
					'Clear description of what you need and why, shown directly to the user (e.g. "Enter the 6-digit activation code just emailed to test@example.com").',
			},
		},
		required: ['prompt'],
	},
	async execute({prompt}, context) {
		if (!context.hasExploredWithBrowser()) {
			return {
				ok: false,
				error:
					'Explore this flow live with browser_action first — navigate to the relevant page and attempt the steps leading up to this point — before asking the user for anything. Do not request input before you have actually attempted a browser_action call this run.',
			};
		}
