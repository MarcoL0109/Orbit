...
export const confirmOutcomeTool: ToolDefinition<
	ConfirmOutcomeArgs,
	ConfirmOutcomeResult
> = {
	...
	parameters: {
		...
		properties: {
			feature: {
				type: 'string',
				description:
					"The exact feature name you will use for this result in report_result — must match precisely, since that's how your confirmation gets connected to it.",
			},
			whatWasDone: {
				type: 'string',
				description:
					'A clear, specific summary of the actions you took — what you tested and how (e.g. "Wrote a Playwright test that creates 2 questions, then drags the first onto the second to reorder them").',
			},
			output: {
				type: 'string',
				description:
					'The actual evidence — the real error message, test output, or observed behavior — not your own interpretation of it. Show the user what you saw, not what you concluded from it.',
			},
		},
		required: ['feature', 'whatWasDone', 'output'],
	},
	async execute({feature, whatWasDone, output}, context) {
		const outcome = await context.requestOutcomeConfirmation(
			feature,
			whatWasDone,
			output,
		);
