...
} from '../../projects/graphifyGraph.js';
	...
	parameters: {
		...
		properties: {
			label: {
				type: 'string',
				description:
					'The function, class, or file name to look up, e.g. "runTestingAgent" or "agent.ts". Matching is forgiving (case-insensitive, ignores a trailing "()", falls back to substring) — if more than one symbol matches, they are listed back so you can retry with a more specific label.',
			},
		},
		required: ['label'],
	},
	async execute({label}, context) {
		const graph = loadGraphifyGraph(context.projectRoot);
		if (!graph) {
			return {
				ok: false,
				error:
					'No code knowledge graph found for this project — explain_symbol is only available when graphify scan mode is active. Use read_file instead.',
			};
		}
		...
		if (matches.length > 1) {
			...
			const candidates = shown
				.map(
					node => `${node.label} (${node.source_file}:${node.source_location})`,
				)
				.join(', ');
			const remainder = matches.length - shown.length;
			return {
				ok: false,
				error: `Multiple symbols match "${label}": ${candidates}${
					remainder > 0 ? `, and ${remainder} more` : ''
				}. Call again with a more specific label.`,
			};
		}
