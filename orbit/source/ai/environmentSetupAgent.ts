...
const environmentSetupToolRegistry: Array<
	ToolDefinition<any, any, EnvironmentSetupContext>
> = [readFileTool, runCommandTool, signalEnvironmentReadyTool];

function buildSystemPrompt(
	context: EnvironmentSetupContext,
	documentedInstructions: string | null,
): string {
	return `You are Orbit's environment setup agent. Your only job: get this project's dev environment running and reachable at ${
		context.orbitConfig.baseUrl
	}, then call signal_environment_ready.
...
- A command meant to keep running (a dev server, "docker compose up" without -d) must background itself so run_command returns rather than hanging — append &, use nohup, or a daemon/-d flag. Don't wrap it in a login shell to do this ("bash -lc '...'", "zsh -lc '...'") — a login shell sources its own separate profile, which can resolve a different version of node/python/whatever than the one already confirmed to work in the commands you've been running, and that mismatch fails in a way that looks like the command itself is broken. A plain "cmd &" or "nohup cmd > log 2>&1 &" inherits this session's own environment exactly and doesn't have that risk.
...
- Before calling signal_environment_ready: if this project has any backend/API separate from a static frontend, use run_command to curl a real, data-backed route on it (not baseUrl itself) and pass what you found as apiCheck. A frontend process responding is not proof anything behind it works — it can serve a page perfectly while every API call that page makes fails, and that's invisible from baseUrl alone. A route that reads from a database also transitively proves the database is reachable, without you needing to touch it directly. Only skip this (pass apiCheck: null) when the project genuinely has no separate backend to check.
- Beyond that one real check, call signal_environment_ready as soon as you believe the environment is up — do not try to perfectly verify everything yourself, since baseUrl reachability is independently re-checked afterward regardless.
...
export async function runEnvironmentSetupAgent(
	...
	const documentedInstructions = readEnvironmentSetupInstructions(
		context.projectRoot,
	);
	...
	let nextInput: string | ResponseInputItem[] =
		"Get this project's dev environment running and reachable.";
	...
	for (let stepCount = 0; stepCount < maxSteps; stepCount++) {
		...
		const signalCall = turn.dispatchedCalls.find(
			call => call.name === 'signal_environment_ready',
		);
		if (signalCall && signalCall.result.ok) {
			const data = signalCall.result.data as {
				notes: string;
			...
			return {
				status: 'signaled',
				notes: data.notes,
				setupProcedure: data.setupProcedure ?? undefined,
				steps,
			};
		}
