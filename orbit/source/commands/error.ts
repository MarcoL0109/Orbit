...
export function checkTaskInProgress(context: CommandContext): OrbitError | null {
    return context.isThinking ? {kind: 'task-in-progress'} : null;
}

export function checkArgCount(rule: ArgCountRule, given: number, usage: string): OrbitError | null {
    if ('exact' in rule) {
    ...
    return given >= rule.min
        ? null
        : {kind: 'invalid-arg-count', usage, expected: `at least ${rule.min}`, given};
}
