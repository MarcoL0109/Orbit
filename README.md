...
For a project Orbit shouldn't read at all — not "won't," structurally can't. Blind mode explores an already-running app purely through a live browser, from a workspace Orbit creates for itself outside the app's own directory. `read_file` and `explain_symbol` aren't just discouraged in this mode, they're removed from the tool set entirely — the model has no way to call them even if it tried. That holds regardless of `scanMode`: `/config`'s scan-mode field refuses to be changed on a blind project at all, and even a graph that somehow ended up on disk anyway wouldn't be enough on its own to bring `explain_symbol` back — the check is structural, not "scanMode happens to be unset." There's no project scan, either, and Orbit never tries to bring the app up itself if it isn't reachable: a blind project has no source for a setup agent to discover a startup sequence from, so an unreachable target fails fast with a clear message instead of guessing.
